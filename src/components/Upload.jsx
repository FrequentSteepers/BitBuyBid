    ...
    return (
      <div style={{width: '100%'}}>
        <form style={{paddingBottom: '10px'}} onSubmit={(e) => this.submitImage(e, this.props.user.id)}>
          <h2 style={{color: 'white', textAlign: 'center'}}>Upload a new profile picture</h2>
          <div style={{position: 'relative', display: 'inline-block'}}>
            <label htmlFor="upload" style={{display: 'block', padding: '10px', color: 'teal', background: 'white', borderRadius: '.2em', transition: 'background .3s', left: '0px'}}>
              Select
            </label>
          ...
          <FlatButton onClick={(e) => this.submitImage(e, this.props.user.id)} style={{position: 'absolute', right: '10px', bottom: '10px', backgroundColor: 'white', color: 'teal', fontStyle: 'italic'}}>Submit</FlatButton>
        </form>
      </div>
    );
  }
